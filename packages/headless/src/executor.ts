...
export interface HeadlessRepository{
  ...
  updatePage(id:string,patch:Partial<Pick<Page,"title"|"icon"|"stateId"|"categoryId"|"priority"|"scheduledAt"|"durationMinutes"|"calendarColor"|"description"|"childView"|"isPinned"|"isArchived">>):Promise<void>;
...
export class HeadlessExecutor{
  ...
  private async pageCreate(input:Input){const id=await this.repository.createPage({title:input.title as string,parentId:input.parentId as string|null,...optionalPatch(input,["stateId","childView"])});try{const patch=optionalPatch(input,["icon","categoryId","priority","scheduledAt","durationMinutes","calendarColor","description","isPinned"]);if(Object.keys(patch).length)await this.repository.updatePage(id,patch);if(input.blocks)await this.repository.saveDocument(id,{type:"doc",content:input.blocks as BlockNodeContent[]});return await this.pageMetadata(id);}catch(cause){await this.repository.deletePage(id).catch(()=>undefined);throw cause;}}
  private async pageUpdate(input:Input){await this.repository.updatePage(input.pageId as string,optionalPatch(input,["title","icon","stateId","categoryId","priority","scheduledAt","durationMinutes","calendarColor","description","childView","isPinned","isArchived"]));return this.pageMetadata(input.pageId as string);}
